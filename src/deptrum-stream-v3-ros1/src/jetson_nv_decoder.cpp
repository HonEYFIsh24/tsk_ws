...
bool JetsonNvJpegDecoder::Decode(const std::shared_ptr<dt::ColorFrame> &frame, uint8_t *dest) {
  if (!IsValidJpeg(frame)) {
    ROS_ERROR_STREAM("Invalid JPEG");
    return false;
  }
  uint32_t pixfmt = 0;
  auto *data = static_cast<uint8_t *>(frame->GetData());
  uint32_t width = 0;
  uint32_t height = 0;
  auto data_size = frame->GetDataSize();
