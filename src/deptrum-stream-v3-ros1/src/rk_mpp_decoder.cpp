...
bool RkMjpegDecoder::Decode(const std::shared_ptr<dt::ColorFrame> &frame, uint8_t *dest) {
  MPP_RET ret = MPP_OK;
  memset(data_buffer_, 0, width_ * height_ * 3);
  memcpy(data_buffer_, frame->GetData(), frame->GetDataSize());
  mpp_packet_set_pos(mpp_packet_, data_buffer_);
  mpp_packet_set_length(mpp_packet_, frame->GetDataSize());
