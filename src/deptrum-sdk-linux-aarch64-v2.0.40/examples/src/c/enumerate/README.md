...
This is a enumerate guide to get device streams profile information using the Deptrum SDK C API.
...
Get stream profile list, then different output formats are formulated according to sensor type. For example, if sensor type = 'kDtSensorColor', need to print stream type、stream format、stream resolution、stream fps、stream index.
...
```c
// Get device name.
const char *dev_name = dt_device_get_device_info(device, DT_CAMERA_INFO_NAME, &error);
...
If you are on Windows, you can switch to the directory `deptrum-dev/build/win_XX/bin` to find the `dt_enumerate_c.exe`.

If you are on linux, you can switch to the directory `deptrum-dev/build/linux_XX/bin` to find the `dt_enumerate_c`.
