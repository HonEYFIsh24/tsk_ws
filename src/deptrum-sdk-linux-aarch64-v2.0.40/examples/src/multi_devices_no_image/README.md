# C++ Sample：dt_multi_devices_no_image

## Overview

In this sample, users can connect multiple camera devices and get color and depth images of different cameras with no image.
...
1. StartStream DeviceIndex is used to identify different devices, and map is used to store the color and depth stream of different devices.
    ...
    for(auto &item: pipes) {
        ...
        pipe->Start(config, [deviceIndex](std::shared_ptr<dt::FrameSet> frameSet) {
