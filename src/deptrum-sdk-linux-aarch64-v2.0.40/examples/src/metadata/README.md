...
1. Create an dt::Pipeline object, and start the pipeline.
    ...
    for(uint32_t j = 0; J < static_cast<uint32_t>(metadataCount); j++) {
        ...
        if(frame->HasMetadata(static_cast<DtFrameMetadataType>(j))) {
            std::cout << "metadata type: " << std::left << std::setw(50) << metadataTypeMap[j]
                      << " metadata value: " << frame->GetMetadataValue(static_cast<DtFrameMetadataType>(j)) << std::endl;
        }
    }
    ```
