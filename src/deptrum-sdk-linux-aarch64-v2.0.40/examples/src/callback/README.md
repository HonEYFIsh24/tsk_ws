...
2. Get only the sensor for the VideoStream,enable the stream from these sensor.
    ...
    ```c++
        for(uint32_t index = 0; index < sensorList->GetCount(); index++) {
                ...
                DtSensorType sensorType = sensorList->GetSensorType(index);

                // exclude non-video sensor type
                if(!dt::TypeHelper::IsVideoSensorType(sensorType)) {
                    continue;
                }
...
If you are on Windows, you can switch to the directory `deptrum-dev/build/win_XX/bin` to find the `dt_callback.exe`.

If you are on linux, you can switch to the directory `deptrum-dev/build/linux_XX/bin` to find the `dt_callback`.
