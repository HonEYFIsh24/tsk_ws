...
std::map<int, std::shared_ptr<dt::Frame>> framesets;
std::mutex frameset_mutex;
std::condition_variable frameset_cv;
...
int main() try {
  ...
  dt_utils::window app(1280, 720, "MultiDevice");
  ...
  while (app) {
    ...
    frameset_cv.wait(lock, [&] { return framesets.size() == dev_count || !is_running; });
    if (!is_running) {
      break;
    }
