...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  if (b_first_frame)
  {
    ...
    mean_gyr << gyr_acc.x, gyr_acc.y, gyr_acc.z;//提取第一帧数据作为初始均值
  ...
  for (const auto &imu : meas.imu)
  {
    ...
    mean_gyr += (cur_gyr - mean_gyr) / N;//计算加速度计和陀螺仪的均值
  ...
  IMU_mean_acc_norm = mean_acc.norm();//计算平均加速度的模长
  state_inout.gravity = -mean_acc / mean_acc.norm() * G_m_s2;//计算重力方向向量
...
void ImuProcess::Forward_without_imu(LidarMeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  if (b_first_frame)
  {
    dt = 0.1;//首帧将时间间隔初始化为0.1秒
    b_first_frame = false;
  }
  else { dt = pcl_beg_time - time_last_scan; }//非首帧使用点云起始时间减去上一帧的点云结束时间
  ...
  F_x.block<3, 3>(0, 0) = Exp(state_inout.bias_g, -dt);//表示陀螺仪偏置对旋转的影响
  F_x.block<3, 3>(0, 10) = Eye3d * dt;
  F_x.block<3, 3>(3, 7) = Eye3d * dt;//表示角速度和加速度对位置和速度的影响
  ...
  //           << std::endl;
  state_inout.cov = F_x * state_inout.cov * F_x.transpose() + cov_w;//使用离散时间状态转移公式更新状态协方差矩阵
  // std::cout << "cov_w:" << cov_w.diagonal().transpose() << std::endl;
  // std::cout << "after propagete:" << state_inout.cov.diagonal().transpose()
  //           << std::endl;
  state_inout.rot_end = state_inout.rot_end * Exp_f;//更新状态的旋转矩阵
  state_inout.pos_end = state_inout.pos_end + state_inout.vel_end * dt;//更新状态的位置
  ...
  if (lidar_type != L515)
  {
    ...
    {
        dt_j= pcl_end_offset_time - it_pcl->curvature/double(1000);//计算当前点云的时间偏移量
        M3D R_jk(Exp(state_inout.bias_g, - dt_j));//计算当前点云的旋转矩阵
        V3D P_j(it_pcl->x, it_pcl->y, it_pcl->z);//获取当前点云的坐标
        // Using rotation and translation to un-distort points
        V3D p_jk;
        p_jk = - state_inout.rot_end.transpose() * state_inout.vel_end * dt_j;//在时间偏移 dt_j 下的旋转补偿，使用 Exp 函数计算
  
        V3D P_compensate =  R_jk * P_j + p_jk;//提取坐标
  
        /// save Undistorted points and their rotation
        it_pcl->x = P_compensate(0);
        it_pcl->y = P_compensate(1);
        it_pcl->z = P_compensate(2);//存储去畸变后的点云坐标
    }
  }
}//该函数的目的是对点云进行前向传播，更新状态和协方差矩阵，并对点云进行去畸变处理。在IMU不可用的情况下，使用之前的估计状态进行预测或补偿
 //使用类似于常值模型的方式来更新状态和协方差矩阵，并对点云进行去畸变处理。这种方法有一定误差，但是可在无IMU数据的情况下维持SLAM去点云畸变的基本能力
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      if (tail->header.stamp.toSec() < prop_beg_time) continue;//如果尾部IMU时间早于传播开始时间，则跳过该IMU数据
      ...
      angvel_avr -= state_inout.bias_g;//矫正偏置
      acc_avr = acc_avr * G_m_s2 / mean_acc.norm() - state_inout.bias_a;//将加速度归一化带重力加速度单位，再减去线性加速度的偏置

      //计算时间间隔和偏置
      if (head->header.stamp.toSec() < prop_beg_time)//第一IMU数据的时间戳小于传播开始时间
      {
        // printf("00 \n");
        dt = tail->header.stamp.toSec() - last_prop_end_time;
        offs_t = tail->header.stamp.toSec() - prop_beg_time;
      }
      else if (i != v_imu.size() - 2)//中间数据
      {
        // printf("11 \n");
      ...
      F_x.block<3, 3>(0, 0) = Exp(angvel_avr, -dt);//更新旋转部分
  ...
  if (lidar_meas.lio_vio_flg == LIO)
  {
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)///外层遍历imu姿态数据
    {
      auto head = it_kp - 1;
      auto tail = it_kp;
      ...
      for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--)//内层遍历点云数据
      {
        dt = it_pcl->curvature / double(1000) - head->offset_time;

        /* Transform to the 'end' frame */
        M3D R_i(R_imu * Exp(angvel_avr, dt));//计算角速度的指数映射，并计算得到点云在IMU坐标系中的旋转变换
        V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt - state_inout.pos_end);//使用IMU的位移和速度计算点云在IMU坐标系中的平移变换
        ...
        //补偿点云位置，将点从IMU坐标系转换到全局坐标系
        /// save Undistorted points and their rotation
        it_pcl->x = P_compensate(0);
        it_pcl->y = P_compensate(1);
        it_pcl->z = P_compensate(2);//更新去畸变后的点云坐标

        if (it_pcl == pcl_wait_proc.points.begin()) break;//直到又循环到点云的起始点
      }
    }
    pcl_out = pcl_wait_proc;//更新输出点云
    pcl_wait_proc.clear();
    IMUpose.clear();
  }
