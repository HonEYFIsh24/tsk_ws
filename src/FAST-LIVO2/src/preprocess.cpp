...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.3), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS = 6;
  group_size = 8;
...
void Preprocess::xt32_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      // ==== 近点剔除：在入栈与时间戳计算等处理前就跳过过近点 ====
      {
        const double x = pl_orig.points[i].x;
        const double y = pl_orig.points[i].y;
        const double z = pl_orig.points[i].z;
        const double dist2 = x * x + y * y + z * z;
        if (dist2 < 0.09) continue;  // 使用已有的 blind_sqr 阈值
      }
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      if (i % point_filter_num == 0)
      {
        ...
        {
          pl_surf.points.push_back(added_pt);
          // printf("blind_sqr: %lf, added_pt.x: %lf, added_pt.y: %lf, added_pt.z: %lf \n",
          //        blind_sqr, added_pt.x, added_pt.y, added_pt.z);
          // pl_orig.points[i].t);
        }
      }
    }
