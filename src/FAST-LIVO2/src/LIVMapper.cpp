...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  if (imu_buffer.empty() && imu_en) return false;//检查数据是否正常

  switch (slam_mode_)
  {
  case ONLY_LIO:
  {
    if (meas.last_lio_update_time < 0.0) meas.last_lio_update_time = lid_header_time_buffer.front();//首帧LIO更新时间设置为 lidar的第一个点云时间
    if (!lidar_pushed)
    {
    ...
    m.imu.clear();
    m.lio_time = meas.lidar_frame_end_time;//LIO时间设置为雷达扫描结束时间
    mtx_buffer.lock();//打开互斥锁，开始处理IMU数据
    while (!imu_buffer.empty())
    {
      if (imu_buffer.front()->header.stamp.toSec() > meas.lidar_frame_end_time) break;//提取时间戳早于或等于 lidar_frame_end_time 的IMU数据
      m.imu.push_back(imu_buffer.front());//存储至m.imu中
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      if (img_capture_time < meas.last_lio_update_time + 0.00001)//丢弃早于最近一次LIO更新时间的图像帧，因为此图像已经过时或无效
      {
      ...
      if (img_capture_time > lid_newest_time || img_capture_time > imu_newest_time)//若图像数据捕获时间晚于最新的雷达扫描结束时间或最新的IMU时间
                                                                                   //说明图像数据无法与其它传感器数据匹配
      {
      ...
      m.imu.clear();
      m.lio_time = img_capture_time;// LIO时间设置为图像捕获时间
      mtx_buffer.lock();
      while (!imu_buffer.empty())
      {
        if (imu_buffer.front()->header.stamp.toSec() > m.lio_time) break;//跳过时间戳晚于LIO时间（即图像捕获时间）的IMU数据，因为此时数据已超出处理范围，无需继续处理

        if (imu_buffer.front()->header.stamp.toSec() > meas.last_lio_update_time) m.imu.push_back(imu_buffer.front());
                                                                         //若IMU数据的时间戳晚于上一次LIO更新时间，说明该IMU数据是新的，将该IMU数据添加到当前测量组中
      ...
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);//更新下一帧的点云数据到当前帧中
      PointCloudXYZI().swap(*meas.pcl_proc_next);//swap方法是一种高效的清理方法，释放*meas.pcl_proc_next的内存，可减少内存碎片提高效率

      int lid_frame_num = lid_raw_data_buffer.size();
      int max_size = meas.pcl_proc_cur->size() + 24000 * lid_frame_num;//估算当前点云数据的最大大小，考虑到每帧雷达数据可能包含24000个点
      meas.pcl_proc_cur->reserve(max_size);
      meas.pcl_proc_next->reserve(max_size);//预留点云数据的内存空间，避免频繁的内存分配和释放
      // deque<PointCloudXYZI::Ptr> lidar_buffer_tmp;

      while (!lid_raw_data_buffer.empty())
      {
        if (lid_header_time_buffer.front() > img_capture_time) break;//超出图像捕获时间的雷达数据不再处理
        auto pcl(lid_raw_data_buffer.front()->points);
        double frame_header_time(lid_header_time_buffer.front());//分别存储点云帧的点数据和点云帧的时间戳
        float max_offs_time_ms = (m.lio_time - frame_header_time) * 1000.0f;//当前点云帧中点的时间偏移上限，单位为ms

        for (int i = 0; i < pcl.size(); i++)
        {
          auto pt = pcl[i];
          if (pcl[i].curvature < max_offs_time_ms)//若偏移量小于最大偏移量，则存入当前帧
          {
            pt.curvature += (frame_header_time - meas.last_lio_update_time) * 1000.0f;//将点的时间偏移量更新为相对于meas.last_lio_update_time的偏移量
            meas.pcl_proc_cur->points.push_back(pt);
          }
          else//否则放到下一帧进行处理
          {
            pt.curvature += (frame_header_time - m.lio_time) * 1000.0f;//将点的时间偏移量更新为相对于m.lio_time的偏移量
