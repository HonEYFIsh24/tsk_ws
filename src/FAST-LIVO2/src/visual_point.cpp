...
bool VisualPoint::getCloseViewObs(const Vector3d &framepos, Feature *&ftr, const Vector2d &cur_px) const
{
  ...
  Vector3d obs_dir(framepos - pos_);//当前视角方向，framepos 是当前相机的位置，pos_ 是地图点的位置
  obs_dir.normalize();//归一化
  auto min_it = obs_.begin();
  double min_cos_angle = 0;
  for (auto it = obs_.begin(), ite = obs_.end(); it != ite; ++it)//遍历所有观测
  {
    Vector3d dir((*it)->T_f_w_.inverse().translation() - pos_);//从地图点指向该观测相机位置的方向向量，(*it)->T_f_w_.inverse().translation()是观测对应相机的世界坐标位置
    dir.normalize();//归一化
    double cos_angle = obs_dir.dot(dir);//计算当前视角方向与观测方向之间的余弦值，表示视角的接近程度
    if (cos_angle > min_cos_angle)
    {
      min_cos_angle = cos_angle;
      min_it = it;
    }//保存视角最接近的观测，并保存视角方向与观测方向之间的余弦值
  }
  ftr = *min_it;
  ...
  if (min_cos_angle < 0.5) // assume that observations larger than 60° are useless 0.5
  {
    ...
    return false;
  }//如果视角接近度小于0.5（即大于60度），则认为该观测无效。

  return true;
}//从一个地图点（VisualPoint）的观测集合中，找到与给定相机位置（framepos）视角最接近的观测（Feature），保存该观测的指针，返回是否成功获取。
